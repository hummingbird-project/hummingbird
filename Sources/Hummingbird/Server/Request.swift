...
public struct HBRequest: Sendable, HBSendableExtensible {
    ...
    public var applicationContext: HBApplication.Context { self._internal.applicationContext }
    /// Request extensions
    ...
    public init(
        ...
        self.body = body
        self.logger = applicationContext.logger.with(metadataKey: "hb_id", value: .stringConvertible(Self.globalRequestID.loadThenWrappingIncrement(by: 1, ordering: .relaxed)))
    ...
    public func decode<Type: Decodable>(as type: Type.Type) throws -> Type {
        do {
            return try self.applicationContext.decoder.decode(type, from: self)
    ...
    final class _Internal: Sendable {
        internal init(uri: HBURL, version: HTTPVersion, method: HTTPMethod, headers: HTTPHeaders, applicationContext: HBApplication.Context, context: HBRequestContext, endpointPath: String? = nil) {
            self.uri = uri
