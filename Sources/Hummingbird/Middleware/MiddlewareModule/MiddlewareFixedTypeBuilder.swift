...
public enum MiddlewareFixedTypeBuilder<Input, Output, Context> where Input: ~Copyable, Context: ~Copyable {
    ...
    where M0.Input == Input, M0.Output == Output, M0.Context == Context, M0.Input: ~Copyable, M0.Context: ~Copyable {
        m0
    }

    public static func buildBlock<M0: MiddlewareProtocol>(_ m0: M0) -> M0 where M0.Input: ~Copyable, M0.Context: ~Copyable {
        m0
    }

    public static func buildPartialBlock<M0: MiddlewareProtocol>(first: M0) -> M0 where M0.Input: ~Copyable, M0.Context: ~Copyable {
        first
    }
    ...
    where M0.Input == M1.Input, M0.Output == M1.Output, M0.Context == M1.Context, M0.Input: ~Copyable, M0.Context: ~Copyable {
        _Middleware2(m0, m1)
    }

    public static func buildOptional<M0: MiddlewareProtocol>(_ component: M0?) -> _OptionalMiddleware<M0>
    where M0.Input: ~Copyable, M0.Context: ~Copyable {
        _OptionalMiddleware(middleware: component)
    }

    public static func buildEither<M0: MiddlewareProtocol>(
        first content: M0
    ) -> M0 where M0.Input: ~Copyable, M0.Context: ~Copyable {
        content
    }
