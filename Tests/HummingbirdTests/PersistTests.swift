...
struct PersistTests {
    ...
    func createRouter(
        ...
        router.put("/persist/:tag") { request, context -> HTTPResponse.Status in
            ...
            try await persist.set(key: tag, value: String(buffer: .init(buffer.span.bytes)))
            return .ok
        }
        router.put("/persist/:tag/:time") { request, context -> HTTPResponse.Status in
            ...
            let buffer = try await request.body.collect(upTo: .max)
            let tag = try context.parameters.require("tag")
            try await persist.set(key: tag, value: String(buffer: .init(buffer.span.bytes)), expires: .seconds(time))
            return .ok
        }
    ...
    @Test func testCreateGet() async throws {
        ...
        router.put("/create/:tag") { request, context -> HTTPResponse.Status in
            let buffer = try await request.body.collect(upTo: .max)
            let tag = try context.parameters.require("tag")
            try await persist.create(key: tag, value: String(buffer: .init(buffer.span.bytes)))
            return .ok
        }
        let app = Application(responder: router.buildResponder())
        try await app.test(.router) { client in
    ...
    @Test func testCodable() async throws {
        ...
        router.put("/codable/:tag") { request, context -> HTTPResponse.Status in
            ...
            let buffer = try await request.body.collect(upTo: .max)
            try await persist.set(key: tag, value: TestCodable(buffer: String(buffer: .init(buffer.span.bytes))))
            return .ok
        }
