...
final class ApplicationTests: XCTestCase {
    ...
    func testServerHeaders() async throws {
        let app = HBApplicationBuilder(context: HBTestRouterContext.self, configuration: .init(serverName: "TestServer"))
        app.router.get("/hello") { _, _ in
            return "Hello"
        }
        try await app.buildAndTest(.live) { client in
    ...
    func testMaxUploadSize() async throws {
        let app = HBApplicationBuilder(context: HBTestRouterContext.self, configuration: .init(maxUploadSize: 64 * 1024))
